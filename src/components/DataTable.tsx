...
export default function DataTable({ data, onSelect, currentSelection, adminMode, exemptCenters, onToggleExempt, rawData, salaryData, attendance15Data, attendance7Data, rosterData, workHoursHighData, workHoursLowData, outsourcingData }: DataTableProps) {
  ...
  // 非操明细弹窗
  const [nonOpDetail, setNonOpDetail] = useState<{ centerName: string; nonOpCount: number; rosterTotal: number; outsourced: number; nonOpRatio: number; departments?: Record<string, number>; positions?: Record<string, { dept: string; count: number }>; staffingStandard?: { departments: { dept: string; fixedStandard: number; tempStandard: number; actual: number }[]; totalFixedStandard: number; totalTempStandard: number; totalActual: number; posStandards: { dept: string; pos: string; fixedStandard: number; tempStandard: number; actual: number }[] } } | null>(null);
                  ...
                  {nonOpTab === 'dept' && nonOpDetail.staffingStandard && (
                    <div className="border border-[#e8e2d9] rounded-lg overflow-hidden">
                      <div className="grid grid-cols-[1fr_80px_80px_80px] gap-2 px-3 py-2 bg-slate-900 text-[10px] font-bold text-slate-400">
                        <div>部门</div><div className="text-center">固定编制</div><div className="text-center">临时编制</div><div className="text-center">在职人数</div>
                      </div>
                      {nonOpDetail.staffingStandard.departments.map(d => (
                        <div key={d.dept} className="grid grid-cols-[1fr_80px_80px_80px] gap-2 px-3 py-2 border-b border-slate-50 text-[11px] hover:bg-[#f0ebe3]/50">
                          ...
                          <div className="text-center font-mono text-slate-500">{d.fixedStandard}</div>
                          <div className="text-center font-mono text-slate-500">{d.tempStandard}</div>
                      ...
                      <div className="grid grid-cols-[1fr_80px_80px_80px] gap-2 px-3 py-2 bg-[#f0ebe3] text-[11px] font-bold">
                        <div>合计</div>
                        <div className="text-center font-mono text-slate-700">{nonOpDetail.staffingStandard.totalFixedStandard}</div>
                        <div className="text-center font-mono text-slate-700">{nonOpDetail.staffingStandard.totalTempStandard}</div>
                        <div className="text-center font-mono text-slate-800">{nonOpDetail.staffingStandard.totalActual}</div>
                      </div>
                    </div>
                  )}
                  {/* 岗位维度 */}
                  {nonOpTab === 'pos' && nonOpDetail.staffingStandard?.posStandards && (
                    <div className="border border-[#e8e2d9] rounded-lg overflow-hidden">
                      <div className="grid grid-cols-[1fr_80px_80px_80px] gap-2 px-3 py-2 bg-slate-900 text-[10px] font-bold text-slate-400">
                        <div>岗位</div><div className="text-center">固定编制</div><div className="text-center">临时编制</div><div className="text-center">在职人数</div>
                      </div>
                      {(() => {
                        // 按部门分组显示岗位
                        const posStandards = nonOpDetail.staffingStandard!.posStandards;
                        const groupedByDept: Record<string, typeof posStandards> = {};
                        posStandards.forEach(ps => {
                          if (!groupedByDept[ps.dept]) groupedByDept[ps.dept] = [];
                          groupedByDept[ps.dept].push(ps);
                        });

                        const rows: React.ReactNode[] = [];
                        for (const [dept, positions] of Object.entries(groupedByDept)) {
                          // 部门标题行
                          rows.push(
                            <div key={`dept-${dept}`} className="grid grid-cols-[1fr_80px_80px_80px] gap-2 px-3 py-1.5 bg-[#f0ebe3] text-[10px] font-bold text-slate-500">
                              <div>{dept}</div><div></div><div></div><div></div>
                            </div>
                          );
                          // 岗位行
                          for (const ps of positions) {
                            rows.push(
                              <div key={`${dept}-${ps.pos}`} className="grid grid-cols-[1fr_80px_80px_80px] gap-2 px-3 py-2 border-b border-slate-50 text-[11px] hover:bg-[#f0ebe3]/50 pl-6">
                                <div className="font-medium text-slate-700">{ps.pos}</div>
                                <div className="text-center font-mono text-slate-500">{ps.fixedStandard}</div>
                                <div className="text-center font-mono text-slate-500">{ps.tempStandard}</div>
                                <div className="text-center font-mono font-bold text-slate-800">{ps.actual || '—'}</div>
                              </div>
                            );
                          }
                        }
