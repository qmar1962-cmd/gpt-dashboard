...
import { X, TrendingUp, User, ChevronDown } from 'lucide-react';
                    ...
                    {day.abnormalCount > 0 ? (
                      <div className="space-y-1">
                        ...
                        <div className="grid grid-cols-[1fr_auto_auto_auto_auto_auto] gap-x-3 px-3 pb-1 border-b border-amber-50">
                          ...
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-wide text-right w-16">个人日薪</span>
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-wide text-right w-16">岗位均值</span>
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-wide text-right w-16">均值偏离</span>
                        </div>
                        {/* 数据行（每天最多显示20条） */}
                        {day.details.slice(0, 20).map((detail, idx) => (
                          <div
                            key={idx}
                            className="grid grid-cols-[1fr_auto_auto_auto_auto_auto] gap-x-3 items-center bg-[#faf7f2] rounded-md px-3 py-2 border border-amber-50"
                          >
                            ...
                            <span className="text-[11px] font-mono font-bold text-slate-700 text-right w-12">
                              ×{detail.attendanceCoeff}
                            </span>
                            <span className="text-[11px] font-mono font-black text-slate-800 text-right w-16">
                              {detail.dailySalary.toFixed(1)}
                            </span>
                            <span className="text-[11px] font-mono text-slate-500 text-right w-16">
                        ...
                        {day.details.length > 20 && (
                          <p className="text-[9px] text-slate-400 font-bold text-center py-1">
                            还有 {day.details.length - 20} 人未显示
                          </p>
                        )}
                      </div>
                    ) : (
                      <p className="text-[10px] text-slate-300 font-bold pl-1">无绩效异常人员</p>
                    )}
                  </div>
                ))}
              </div>
