-- GPT 每日通报可视化看板 - Supabase 数据库表结构（修复版）
...
DROP POLICY IF EXISTS "允许所有人写入 leave_plans" ON leave_plans;
DROP POLICY IF EXISTS "允许所有人写入 absence_reasons" ON absence_reasons;
DROP POLICY IF EXISTS "允许所有人写入 center_meta" ON center_meta;
DROP POLICY IF EXISTS "允许所有人写入 group_leaders" ON group_leaders;
